...
#define NUM_FIL14 48
#define NUM_FIL15 24
#define NUM_FIL16 144
#define NUM_FIL17 144
#define NUM_FIL18 24
#define NUM_FIL19 120
#define NUM_FIL20 120
#define NUM_FIL21 24
#define NUM_FIL22 144
#define NUM_FIL23 144
#define NUM_FIL24 40
#define NUM_FIL25 240
#define NUM_FIL26 240
#define NUM_FIL27 40
#define NUM_FIL28 240
#define NUM_FIL29 240
#define NUM_FIL30 48
#define NUM_FIL31 192
#define NUM_FIL32 192
#define NUM_FIL33 48
#define NUM_FIL34 240
#define NUM_FIL35 240
#define NUM_FIL36 96
#define NUM_FIL37 480
#define NUM_FIL38 480
#define NUM_FIL39 96
#define NUM_FIL40 384
#define NUM_FIL41 384
#define NUM_FIL42 96
#define NUM_FIL43 288
#define NUM_FIL44 288
#define NUM_FIL45 160
...
void mcunet_vww() {
	...
	for (int iter = 0; iter < SAMPLES; iter++){
		...
		dw_conv(outp_dim1, f_dim2, outp_dim2, out1, F2, B2, out2, STRIDE2, pad_2, SB2, MV2, SV2, 1);
		pw_conv(outp_dim2, f_dim3, outp_dim3, out2, F3, B3, out3, STRIDE3, pad_3, SB3, MV3, SV3, 0);
		pw_conv(outp_dim3, f_dim4, outp_dim4, out3, F4, B4, out4, STRIDE4, pad_4, SB4, MV4, SV4, 1);
		dw_conv(outp_dim4, f_dim5, outp_dim5, out4, F5, B5, out5, STRIDE5, pad_5, SB5, MV5, SV5, 1);
		pw_conv(outp_dim5, f_dim6, outp_dim6, out5, F6, B6, out6, STRIDE6, pad_6, SB6, MV6, SV6, 0);
		pw_conv(outp_dim6, f_dim7, outp_dim7, out6, F7, B7, out7, STRIDE7, pad_7, SB7, MV7, SV7, 1);
		dw_conv(outp_dim7, f_dim8, outp_dim8, out7, F8, B8, out8, STRIDE8, pad_8, SB8, MV8, SV8, 1);
		pw_conv(outp_dim8, f_dim9, outp_dim9, out8, F9, B9, out9, STRIDE9, pad_9, SB9, MV9, SV9, 0);
		shortcut(outp_dim10, out9, out6, out10);

		pw_conv(outp_dim10, f_dim11, outp_dim11, out10, F10, B10, out11, STRIDE10, pad_11, SB10, MV10, SV10, 1);
		dw_conv(outp_dim11, f_dim12, outp_dim12, out11, F11, B11, out12, STRIDE11, pad_12, SB11, MV11, SV11, 1);
		pw_conv(outp_dim12, f_dim13, outp_dim13, out12, F12, B12, out13, STRIDE12, pad_13, SB12, MV12, SV12, 0);
		shortcut(outp_dim14, out13, out10, out14);

		pw_conv(outp_dim14, f_dim15, outp_dim15, out14, F13, B13, out15, STRIDE13, pad_15, SB13, MV13, SV13, 1);
		dw_conv(outp_dim15, f_dim16, outp_dim16, out15, F14, B14, out16, STRIDE14, pad_16, SB14, MV14, SV14, 1);
		pw_conv(outp_dim16, f_dim17, outp_dim17, out16, F15, B15, out17, STRIDE15, pad_17, SB15, MV15, SV15, 0);
		pw_conv(outp_dim17, f_dim18, outp_dim18, out17, F16, B16, out18, STRIDE16, pad_18, SB16, MV16, SV16, 1);
		dw_conv(outp_dim18, f_dim19, outp_dim19, out18, F17, B17, out19, STRIDE17, pad_19, SB17, MV17, SV17, 1);
		pw_conv(outp_dim19, f_dim20, outp_dim20, out19, F18, B18, out20, STRIDE18, pad_20, SB18, MV18, SV18, 0);
		shortcut(outp_dim21, out20, out17, out21);

		pw_conv(outp_dim21, f_dim22, outp_dim22, out21, F19, B19, out22, STRIDE19, pad_22, SB19, MV19, SV19, 1);
		dw_conv(outp_dim22, f_dim23, outp_dim23, out22, F20, B20, out23, STRIDE20, pad_23, SB20, MV20, SV20, 1);
		pw_conv(outp_dim23, f_dim24, outp_dim24, out23, F21, B21, out24, STRIDE21, pad_24, SB21, MV21, SV21, 0);
		shortcut(outp_dim25, out24, out21, out25);

		pw_conv(outp_dim25, f_dim26, outp_dim26, out25, F22, B22, out26, STRIDE22, pad_26, SB22, MV22, SV22, 1);
		dw_conv(outp_dim26, f_dim27, outp_dim27, out26, F23, B23, out27, STRIDE23, pad_27, SB23, MV23, SV23, 1);
		pw_conv(outp_dim27, f_dim28, outp_dim28, out27, F24, B24, out28, STRIDE24, pad_28, SB24, MV24, SV24, 0);
		pw_conv(outp_dim28, f_dim29, outp_dim29, out28, F25, B25, out29, STRIDE25, pad_29, SB25, MV25, SV25, 1);
		dw_conv(outp_dim29, f_dim30, outp_dim30, out29, F26, B26, out30, STRIDE26, pad_30, SB26, MV26, SV26, 1);
		pw_conv(outp_dim30, f_dim31, outp_dim31, out30, F27, B27, out31, STRIDE27, pad_31, SB27, MV27, SV27, 0);
		shortcut(outp_dim32, out31, out28, out32);

		pw_conv(outp_dim32, f_dim33, outp_dim33, out32, F28, B28, out33, STRIDE28, pad_33, SB28, MV28, SV28, 1);
		dw_conv(outp_dim33, f_dim34, outp_dim34, out33, F29, B29, out34, STRIDE29, pad_34, SB29, MV29, SV29, 1);
		pw_conv(outp_dim34, f_dim35, outp_dim35, out34, F30, B30, out35, STRIDE30, pad_35, SB30, MV30, SV30, 0);
		pw_conv(outp_dim35, f_dim36, outp_dim36, out35, F31, B31, out36, STRIDE31, pad_36, SB31, MV31, SV31, 1);
		dw_conv(outp_dim36, f_dim37, outp_dim37, out36, F32, B32, out37, STRIDE32, pad_37, SB32, MV32, SV32, 1);
		pw_conv(outp_dim37, f_dim38, outp_dim38, out37, F33, B33, out38, STRIDE33, pad_38, SB33, MV33, SV33, 0);
		shortcut(outp_dim39, out38, out35, out39);

		pw_conv(outp_dim39, f_dim40, outp_dim40, out39, F34, B34, out40, STRIDE34, pad_40, SB34, MV34, SV34, 1);
		dw_conv(outp_dim40, f_dim41, outp_dim41, out40, F35, B35, out41, STRIDE35, pad_41, SB35, MV35, SV35, 1);
		pw_conv(outp_dim41, f_dim42, outp_dim42, out41, F36, B36, out42, STRIDE36, pad_42, SB36, MV36, SV36, 0);
		pw_conv(outp_dim42, f_dim43, outp_dim43, out42, F37, B37, out43, STRIDE37, pad_43, SB37, MV37, SV37, 1);
		dw_conv(outp_dim43, f_dim44, outp_dim44, out43, F38, B38, out44, STRIDE38, pad_44, SB38, MV38, SV38, 1);
		pw_conv(outp_dim44, f_dim45, outp_dim45, out44, F39, B39, out45, STRIDE39, pad_45, SB39, MV39, SV39, 0);
		shortcut(outp_dim46, out45, out42, out46);

		pw_conv(outp_dim46, f_dim47, outp_dim47, out46, F40, B40, out47, STRIDE40, pad_47, SB40, MV40, SV40, 1);
		dw_conv(outp_dim47, f_dim48, outp_dim48, out47, F41, B41, out48, STRIDE41, pad_48, SB41, MV41, SV41, 1);
		pw_conv(outp_dim48, f_dim49, outp_dim49, out48, F42, B42, out49, STRIDE42, pad_49, SB42, MV42, SV42, 0);
		shortcut(outp_dim50, out49, out46, out50);

		pw_conv(outp_dim50, f_dim51, outp_dim51, out50, F43, B43, out51, STRIDE43, pad_51, SB43, MV43, SV43, 1);
		dw_conv(outp_dim51, f_dim52, outp_dim52, out51, F44, B44, out52, STRIDE44, pad_52, SB44, MV44, SV44, 1);
		pw_conv(outp_dim52, f_dim53, outp_dim53, out52, F45, B45, out53, STRIDE45, pad_53, SB45, MV45, SV45, 0);
		avgpool2(outp_dim53, outp_dim54, out53, out54, POOL_SIZE1, POOL_STRIDE1);

		flatten(outp_dim54, out54, out55);

		mlp_layer(out55, out, flatten_dim, OUT_DIM, W1, B46, SB46, MV46, SV46, 1);
		pcount_enable(0);
